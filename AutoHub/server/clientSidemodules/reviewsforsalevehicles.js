...
router.get('/getSaleVehicleDetails/:selectedVehicleId', async (req, res) => {
  const { selectedVehicleId } = req.params;

  try {
    const findVehicle = await Salevehicle.findById(selectedVehicleId);
...
router.post('/postsalevehiclereviews', authenticate, async (req, res) => {
  try {
    ...
    const newReview = new Salevehiclereviews({
...
router.get('/getReviewsForVehicle/:selectedVehicleId', async (req, res) => {
  const { selectedVehicleId } = req.params;

  try {
    const foundReviews = await Salevehiclereviews.find({ vehicle: selectedVehicleId })
